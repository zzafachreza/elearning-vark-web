...
class Kuis extends CI_Controller{
	...
	function index(){
		...
		if (!isset($_SESSION['username'])) {
			...
			$data['modul'] = $this->modul();
			$this->load->view('header',$data);
			$this->load->view($this->modul().'/edit_syarat');
			$this->load->view('footer');
		}
	}
    ...
    function insert_syarat(){
        $fid_materi = $_POST['fid_materi'];
        $nama_syarat = trim($_POST['nama_syarat']);
        $sql="INSERT INTO data_syarat(fid_materi,nama_syarat) VALUES('$fid_materi','$nama_syarat')";
        
        if($this->db->query($sql)){
                // $this->session->set_flashdata('update', ' Data berhasil di tambah');
                   redirect($this->modul().'/detail/'.$fid_materi); 
        }
    }
      ...
      function update_syarat(){
        ...
        $nama_syarat = trim($_POST['nama_syarat']);
        $sql="UPDATE data_syarat SET nama_syarat='$nama_syarat' WHERE id_syarat='$id_syarat'";
    ...
    function delete_syarat(){
        $fid_materi = $this->uri->segment(3);
        $id_syarat = $this->uri->segment(4);
        $sql="DELETE FROM data_syarat WHERE id_syarat='$id_syarat'";
...
$objWriter->save('php://output');
	...
	function update(){
	      ...
	      $icon_old = $_POST['icon_'.$this->modul().'_old'];
      ...
      if(!empty($_FILES['icon_'.$this->modul()]['name'])){
        
          $myIcon = $this->uploadFoto('icon_'.$this->modul(),date('ymdhis'));
          unlink($icon_old);
        ...
        $sql="UPDATE data_".$this->modul()." SET ";
           ...
           foreach($this->db->query("SHOW COLUMNS from `data_".$this->modul()."` WHERE FIELD !='id_".$this->modul()."'")->result() as $col){
        	    ...
        	    if($data_field=='file_'.$this->modul()){
        	        $data_field = $myFoto;
        	    }else if($data_field=='icon_'.$this->modul()){
        	        $data_field = $myIcon;
        	    }else{
        	         if($data_field=="harga_".$this->modul()){
        	             $data_field = str_replace(",","",$this->input->post($col->Field));
