...
function nilai_add(){
    ...
    $nilai = $data['nilai'];
        ...
        $sql="INSERT INTO data_nilai(kategori,fid_user,nilai) VALUES('$kategori','$fid_user','$nilai')";
...
function soal(){
    ...
    $sql="SELECT * FROM data_soal WHERE kategori='$kategori' ORDER BY nomor*1 ASC";
    $arr = array();
...
function kuis(){
    ...
    $sql="SELECT * FROM data_kuis ORDER BY nomor*1 ASC";
    $arr = array();
...
function update_vark(){
    ...
    $sql="UPDATE data_user SET v='$v',a='$a',r='$r',k='$k' WHERE id='$id'";
    
    if($this->db->query($sql)){
        
        $sqlRow = "SELECT * FROM data_user WHERE id='$id'";
             $arr = $this->db->query($sqlRow)->row_array();
	...
	function register(){
        ...
        $password = sha1($data['password']);
            ...
            if($cek > 0){
                ...
                $this->db->query($sql);
                echo json_encode(array("status"=>200,"message"=>"Selamat, kamu berhasil mendaftar !"));
            }
...
function update_profile(){
    $data = json_decode(file_get_contents('php://input'), true);
        ...
        if(!empty($data['newpassword'])){
             ...
             $sql="UPDATE data_user SET nama_lengkap='$nama_lengkap',username='$username',password='$password',foto_user='$input_user' WHERE id='$id'";
        
            
        }else{
            $sql="UPDATE data_user SET nama_lengkap='$nama_lengkap',username='$username',foto_user='$input_user' WHERE id='$id'";
        }
