        			...
        			<?php echo ucfirst(str_replace("_"," ",$col->Field)) ?> 
        			  ...
        			  <?php 
        			   ...
        			   <select  name="<?php echo $col->Field ?>" class="form-control selectza">
        			       <option <?php echo $data->{$col->Field}=="A"?"selected":"" ?>>A</option>
        			       <option <?php echo $data->{$col->Field}=="B"?"selected":"" ?>>B</option>
        			       <option <?php echo $data->{$col->Field}=="C"?"selected":"" ?>>C</option>
        			       <option <?php echo $data->{$col->Field}=="D"?"selected":"" ?>>D</option>
        			       <option <?php echo $data->{$col->Field}=="E"?"selected":"" ?>>E</option>
        			    ...
        			    <label class="AppLabel">
        			      Kategori
        			       ...
        			       <select  name="<?php echo $col->Field ?>" class="form-control selectza">
        			           ...
        			           <option  <?php echo $data->{$col->Field}=="Visual"?"selected":"" ?> >Visual</option>
        			           <option  <?php echo $data->{$col->Field}=="Auditori"?"selected":"" ?> >Auditori</option>
        			           <option  <?php echo $data->{$col->Field}=="Reading/Writing"?"selected":"" ?> >Reading/Writing</option>
        			           <option  <?php echo $data->{$col->Field}=="Kinesthetic"?"selected":"" ?> >Kinesthetic</option>
        			       <?php 
        			       foreach($this->db->query("SELECT * FROM data_materi WHERE fid_menu=8")->result() as $r){
        			       ?>
        			        <option <?php echo  $data->{$col->Field}==$r->nama_materi?'selected':'' ?> ><?php echo $r->nama_materi ?></option>
